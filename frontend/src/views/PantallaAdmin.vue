...
function abrirNuevoMiembro(evento: Event) {
  ...
  Object.assign(nuevoMiembro, { nombre: '', correo: '', permiso: miembros.value.length ? 'ADULTO' : 'ADMINISTRADOR_FAMILIAR' })
  ...
  <main v-else class="admin-plataforma">
    ...
    <section class="admin-registro" aria-labelledby="titulo-registro-familias">
      ...
      <div v-if="familias.length" class="admin-familias">
        <button v-for="familia in familias" :key="familia.id" type="button" class="admin-familia" :class="{ 'admin-familia--activa': familiaActiva?.id === familia.id }" @click="seleccionarFamilia(familia)"><span class="admin-familia__marca" aria-hidden="true">F</span><span class="admin-familia__datos"><strong>{{ familia.nombre }}</strong><span>{{ familia.zonaHoraria }}</span><small>Creada {{ fecha(familia.creadaEn) }}</small></span><span class="estado">Gestionar</span></button>
      </div>
      ...
      <section v-if="familiaActiva" id="miembros-familia" class="admin-miembros" aria-labelledby="titulo-miembros">
        <div class="admin-miembros__titulo"><div><p class="sobretitulo">{{ familiaActiva.nombre }}</p><h2 id="titulo-miembros">Miembros con acceso</h2></div><button type="button" class="boton-principal" @click="abrirNuevoMiembro">Añadir miembro</button></div>
        ...
        <div v-if="miembros.length" class="admin-miembros__lista">
          <article v-for="miembro in miembros" :key="miembro.perfilId" class="admin-miembro">
            ...
            <span v-if="cuenta(miembro)" class="estado estado--activo">Acceso activo</span><span v-else-if="invitacion(miembro)" class="estado">{{ invitacion(miembro)?.estado.toLowerCase() }}</span><span v-else class="estado">Sin invitación</span>
            <div class="admin-miembro__acciones"><button v-if="cuenta(miembro)" type="button" class="boton-secundario" :disabled="cargando" @click="restablecer(miembro, $event)">Restablecer acceso</button><button v-else-if="invitacion(miembro)?.estado === 'PENDIENTE'" type="button" class="boton-secundario" :disabled="cargando" @click="revocar(invitacion(miembro)!)">Revocar invitación</button><button v-else type="button" class="boton-secundario" :disabled="cargando" @click="abrirInvitacion(miembro, $event)">Generar invitación</button></div>
          </article>
        </div>
