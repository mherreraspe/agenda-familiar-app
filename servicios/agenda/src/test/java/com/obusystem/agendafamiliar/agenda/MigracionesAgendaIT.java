...
class MigracionesAgendaIT {
    ...
    void rlsOcultaOcurrenciasYRevisionDeOtraFamilia() {
        ...
        jdbc.execute("GRANT SELECT ON ocurrencias_tratamiento, elementos_revision, acciones_tratamiento, lugares_familia, palabras_clave, recurrencias_agenda, acciones_agenda, archivos_familia, ubicaciones_objetos, objetos_familia, idempotencia_objetos TO prueba_rls");
        ...
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM archivos_familia", Integer.class)).isZero();
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM ubicaciones_objetos", Integer.class)).isZero();
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM objetos_familia", Integer.class)).isZero();
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM idempotencia_objetos", Integer.class)).isZero();
    }
